...
export class AuthService {
  ...
  loginWithNativeAuth(credentials: { login: string; password: string }): Observable<AuthResponse> {
    const url = `${this.baseUrl}/auth/gtt`;
    return this.http.post<AuthResponse>(url, credentials, this.getHttpOptions()).pipe(
      tap((response: AuthResponse) => {
        ...
        console.log('Native login response:', response);
      })
    );
  }
   ...
   */
  loginWithGoogleToken(googleToken: string): Observable<AuthResponse> {
